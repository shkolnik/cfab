...
package upgrade included) keeps the leg instead: deleting it would flush the kernel neighbor
entries that are this member's record of which VMs live here, and an idle VM would go
unannounced until it next sent something.
...
- **Stopping the service removes** the same things with one exception: the leg stays, with this
  member's own address and the bridge's vid, so a restart keeps its neighbor entries and the VMs
  on it stay announced. The anycast `gw` address still goes, so a stopped member stops answering
  for the gateway and VMs re-resolve it to a live one.
