...
/// The bonding driver's own per-port link state (`IFLA_BOND_SLAVE_MII_STATUS`, which the kernel
/// also publishes as text in `/sys/class/net/<port>/bonding_slave/mii_status`). The four states
/// are the driver's whole ladder; `Unknown` carries the raw number of a fifth a later driver may
/// grow, which is loud and not up, never a guess. Built in one place, `netlink::port_state`.
