#!/bin/bash
# cfab-entrypoint.sh — container PID 1: up, status once (informational — a member that cannot
...
# up/status/wait member lifecycle below.
if [ $# -gt 0 ]; then
	exec "$@"
fi
...
cfab status --permissive --wait "${CFAB_STATUS_WAIT:-90}" || echo "cfab: status is FAILED or DOWN — inspect with: docker exec <container> cat /run/cfab/engine.log" >&2
echo "cfab: running (docker exec <container> cfab status to re-check)"
